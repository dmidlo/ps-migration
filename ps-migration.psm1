...
foreach ($viewFolder in $viewFolders) {
    Get-ChildItem -Recurse "$PSScriptRoot\$viewFolder\*.ps1" -File | ForEach-Object {
        . $_.FullName
    }
}
...
foreach ($controllerFolder in $controllerFolders) {
    Get-ChildItem -Recurse "$PSScriptRoot\$controllerFolder\*.ps1" -File | ForEach-Object {
        . $_.FullName
    }
}
...
foreach ($scriptsFolder in $scriptsFolders) {
    Get-ChildItem -Recurse "$PSScriptRoot\$scriptsFolder\*.ps1" -File | ForEach-Object {
        . $_.FullName
    }
}
...
foreach ($dbFolder in $dbFolders) {
    Get-ChildItem -Recurse "$PSScriptRoot\$dbFolder\*.ps1" -File | ForEach-Object {
        . $_.FullName
    }
}
...
foreach ($testFolder in $testFolders) {
    Get-ChildItem -Recurse "$PSScriptRoot\$testFolder\*.ps1" -File | ForEach-Object {
        . $_.FullName
    }
}
...
# $vendoredPath = Join-Path $PSScriptRoot 'bin/your-vendored.exe'
