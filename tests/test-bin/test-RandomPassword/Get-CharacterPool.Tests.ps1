Describe "Get-CharacterPool" {
    ...
    Context "Get-CharacterPool: Valid Character Pool Generation" -Tag 'GetCharacterPool','active'{
        $testCases = @(
    ...
    Context "Get-CharacterPool: Edge Cases & Error Handling" {
        It "Get-CharacterPool TC07: Throws an error when all flags are '`$false'" -Tag 'GetCharacterPool', 'active' {
            { Get-CharacterPool } |
            Should -Throw -ExpectedMessage "At least one character type must be included."
        }

        It "Get-CharacterPool TC08: Rejects non-boolean inputs with appropriate error" {
            { Get-CharacterPool -IncludeUpper "yes" -IncludeLower 123 } |
            Should -Throw -ErrorType [System.Management.Automation.ParameterBindingException] -ErrorMessage "*cannot be processed because it is not a valid Boolean value*"
        }

        It "Get-CharacterPool TC09: Ensures case sensitivity of output" -Tag 'GetCharacterPool','active' {
        ...
        It "Get-CharacterPool TC10: Maintains expected character order" -Tag 'GetCharacterPool','active' {
        ...
        It "Get-CharacterPool TC11: Does not contain duplicate characters" -Tag 'GetCharacterPool','active' {
        ...
        It "Get-CharacterPool TC12: Does not introduce unexpected whitespace" -Tag 'GetCharacterPool','active' {
            Get-CharacterPool -IncludeUpper $true -IncludeLower $true -IncludeNumeric $true -IncludeSpecial $true |
                Should -Not -Match "\s"
        }
    }
    ...
    Context "Get-CharacterPool Special Character Handling" {
        It "Get-CharacterPool TC13: Correctly escapes special characters" -Tag 'GetCharacterPool','active' {
            $result = Get-CharacterPool -IncludeSpecial $true
            $expectedSpecial = "!@#$%^&*()-_=+[]{}|;:,.<>?/"
            $result | Should -BeExactly $expectedSpecial
        }

        It "Get-CharacterPool TC14: Special characters do not cause unintended regex behavior" -Tag 'GetCharacterPool','active' {
    ...
    Context "Get-CharacterPool Performance & CI/CD Readiness" {
        It "Get-CharacterPool TC15: Executes within a reasonable time limit (median of 10 runs)" -Tag 'GetCharacterPool','active' {
        ...
        It "Get-CharacterPool TC16: Stable execution time across multiple runs (variance ≤ 1.5x median)" -Tag 'GetCharacterPool','active' {
            $times = 1..1000 | ForEach-Object { (Measure-Command { Get-CharacterPool -IncludeUpper -IncludeLower -IncludeNumeric -IncludeSpecial }).TotalMilliseconds }
            $sortedTimes = $times | Sort-Object
            $middleIndex = [math]::Floor($sortedTimes.Count / 2)
            $medianTime = $sortedTimes[$middleIndex]
        ...
        It "Get-CharacterPool TC17: Generates expected length output" -Tag 'GetCharacterPool','active' {
