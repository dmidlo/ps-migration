Describe "Get-CharacterPool" {
    ...
    Context "Get-CharacterPool: Valid Character Pool Generation" -Tag 'Bin','RandomPassword','GetCharacterPool','active'{
        $testCases = @(
        ...
        It "Get-CharacterPool TC07: Generates expected length output" -Tag 'Bin','RandomPassword','GetCharacterPool','active' {
    ...
    Context "Get-CharacterPool: Edge Cases & Error Handling" {
        It "Get-CharacterPool TC08: Throws an error when all flags are '`$false'" -Tag 'Bin','RandomPassword','GetCharacterPool','active' {
            { Get-CharacterPool } |
            Should -Throw -ExpectedMessage "At least one character type must be included."
        }

        It "Get-CharacterPool TC09: Ensures case sensitivity of output" -Tag 'Bin','RandomPassword','GetCharacterPool','active' {
        ...
        It "Get-CharacterPool TC10: Maintains expected character order" -Tag 'Bin','RandomPassword','GetCharacterPool','active' {
        ...
        It "Get-CharacterPool TC11: Does not contain duplicate characters" -Tag 'Bin','RandomPassword','GetCharacterPool','active' {
        ...
        It "Get-CharacterPool TC12: Does not introduce unexpected whitespace" -Tag 'Bin','RandomPassword','GetCharacterPool','active' {
            Get-CharacterPool -IncludeUpper $true -IncludeLower $true -IncludeNumeric $true -IncludeSpecial $true |
                Should -Not -Match "\s"
        }
    }
    ...
    Context "Get-CharacterPool Special Character Handling" {
        It "Get-CharacterPool TC13: Correctly escapes special characters" -Tag 'Bin','RandomPassword','GetCharacterPool','active' {
            $result = Get-CharacterPool -IncludeSpecial $true
            $expectedSpecial = "!@#$%^&*()-_=+[]{}|;:,.<>?/"
            $result | Should -BeExactly $expectedSpecial
        }

        It "Get-CharacterPool TC14: Special characters do not cause unintended regex behavior" -Tag 'Bin','RandomPassword','GetCharacterPool','active' {
            $specialPool = Get-CharacterPool -IncludeSpecial $true
            $specialPool -match "[!@#$%^&*()\[\]{}|;:,.<>?/]" | Should -Be $true
        }
    }

    Context "Get-CharacterPool Performance" {
        It "Get-CharacterPool TC15: Executes within a reasonable time limit (median of 10 runs)" -Tag 'Bin','RandomPassword','GetCharacterPool','active' {
        ...
        It "Get-CharacterPool TC16: Stable execution time across multiple runs (variance ≤ 1.5x median)" -Tag 'Bin','RandomPassword','GetCharacterPool','active' {
            $times = 1..1000 | ForEach-Object { (Measure-Command { Get-CharacterPool -IncludeUpper -IncludeLower -IncludeNumeric -IncludeSpecial }).TotalMilliseconds }
            $sortedTimes = $times | Sort-Object
            $middleIndex = [math]::Floor($sortedTimes.Count / 2)
            $medianTime = $sortedTimes[$middleIndex]
