function New-LiteDbAppendOnlyStore {
    <#
    .SYNOPSIS
    Creates a new append-only LiteDB store object for journaling/versioned documents.

    .DESCRIPTION
    This function constructs a [LiteDbAppendOnlyStore] class instance, which provides
    ...
    $store = New-LiteDbAppendOnlyStore -Database $db -Collection 'Domains'
    $store.ReadAll()
    ...
    process {
        return [LiteDbAppendOnlyStore]::new($Database, $Collection)
    }
}
