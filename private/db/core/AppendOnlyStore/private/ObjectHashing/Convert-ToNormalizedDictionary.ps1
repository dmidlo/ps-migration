function Convert-ToNormalizedDictionary {
    <#
    .SYNOPSIS
    ...
    .DESCRIPTION
        ...
        - Recursively normalizes values using `Convert-ToNormalizedData`.
    ...
    foreach ($key in $sortedKeys) {
        if (-not $IgnoreFields.Contains($key)) {
            $value = $hashTable[$key]
            $normalizedValue = Convert-ToNormalizedData -InputObject $value -IgnoreFields $IgnoreFields
