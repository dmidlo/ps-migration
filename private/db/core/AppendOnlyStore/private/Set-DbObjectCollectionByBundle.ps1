        ...
        try {
            foreach ($version in $dbObject) {
                try {
                    ...
                    ($stagedVersion = $version.VersionId | Get-DbDocumentByVersion -Database $Database -Collection $SourceCollection) | Out-Null
                    ...
                    if ($props -contains '$VersionArcs') {
                        ...
                        foreach ($VersionArc in $version.'$VersionArcs') {
                            ...
                            ($stagedVersionArc = $VersionArc.VersionId | Get-DbDocumentByVersion -Database $Database -Collection $RefCollection) | Out-Null
                            if ($stagedVersionArc.PSObject.Properties.Name -notcontains '$Ref') {
                                ($stagedVersionArc = $VersionArc.VersionId | Get-DbDocumentByVersion -Database $Database -Collection $DestCollection) | Out-Null
                            }
                            ...
                            if ($VersionArc.BundleId -eq $BundleId) {
                                ...
                                ($IsAlreadyPresent_VersionArc = $stagedVersionArc.VersionId | Get-DbDocumentByVersion -Database $Database -Collection $DestCollection) | Out-Null
                    ...
                    if ($props -contains '$BundleArcs') {
                        ...
                        foreach ($BundleArc in $version.'$BundleArcs') {
                            ...
                            ($stagedBundleArc = $BundleArc.VersionId | Get-DbDocumentByVersion -Database $Database -Collection $RefCollection) | Out-Null
                            ...
                            if ($BundleArc.BundleId -eq $BundleId){
                                ...
                                $IsAlreadyPresent_BundleArc = $stagedBundleArc.VersionId | Get-DbDocumentByVersion -Database $Database -Collection $DestCollection
